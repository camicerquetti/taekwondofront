...
export default function AcademiaScreen({ route }) {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userData) {
          ...
          setUserPlan(user.plan || 'basico'); // Asumiendo que el plan está en 'plan'
        }
      } catch (error) {
  ...
  const [filter, setFilter] = useState('nuevos'); // 'nuevos' o 'masVistos'

  useEffect(() => {
    fetch('https://taekwondoitfapp.com/api/auth/tul_contenidos')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setContenidos(data);
        else setError('No se encontraron contenidos');
      })
  ...
  const contenidosFiltrados = contenidos
    .filter(item => item.movimiento_o_academia === 'Academia')
    .filter(item =>
  ...
  onPress={() => {
    if (!userPlan) {
      ...
      return; // Detiene la ejecución si no hay plan
    }

    // Lógica de navegación según el plan
    if (item.tipo_seccion === 'PRO') {
      // Si es un contenido PRO
      if (userPlan === 'pro') {
        // Si el usuario tiene el plan PRO, puede acceder
        navigation.navigate('academia', { contenidoId: item.id });
      } else {
        // Si el usuario no tiene el plan PRO
       
        navigation.navigate('pro'); // Redirige a la pantalla del plan PRO
      }
    } else if (item.tipo_seccion === 'BASICO') {
      // Si es un contenido BASIC
      if (userPlan === 'pro') {
        // Si el usuario tiene el plan PRO, puede acceder
        navigation.navigate('academia', { contenidoId: item.id });
      } else if (userPlan === 'basico') {
        // Si el usuario tiene el plan BASIC, puede acceder
        navigation.navigate('academia', { contenidoId: item.id });
      } else {
        // Si el usuario no tiene plan, se le redirige a la página de login o algo similar
        alert('Este contenido solo está disponible para usuarios registrados.');
      }
    } else {
      // Si es un contenido que no tiene restricción (es un contenido público o no clasificado)
      navigation.navigate('academia', { contenidoId: item.id });
    }
  }}
>
