...
export default function MovimientoScreen({ route }) {
  ...
  const contenidosFiltrados = contenidos
    .filter(item => (item.movimiento_o_academia || '').toLowerCase().trim() === 'do')
    .filter(item =>
  ...
  if (loading) return (
    <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>      <ActivityIndicator size="large" color="#000" />
    </View>
  );

  if (error) return (
    <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>      <Text>{error}</Text>
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Header />
      <Text style={styles.title}>Do - El camino</Text>
      <Text style={styles.description}>
        Principios, valores y el camino interior del Taekwon-Do. Textos reflexivos para quienes buscan entender el arte más allá de la técnica.
      </Text>
      ...
      <View style={styles.filterBar}>
        <Text style={styles.ordenadoText}>Ordenado por:</Text>
        <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  filterButtonInactive: {
    ...
    paddingVertical: 8, borderRadius: 10, marginHorizontal: 4,
  },
